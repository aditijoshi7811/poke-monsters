 ...
 */
export function usePokemonCatchMechanic(trainerStore, pokemonStore, emitFn = () => {}) {
  ...
  const attemptThrow = async () => {
    ...
    try {
      ...
      if (random < successRate) {
        ...
        pokemonStore.addEvent(`Success! ${capitalize(current.name)} was caught!`);
        ...
        // Failure
        failureCount.value++;
        pokemonStore.addEvent(` Failed to catch ${capitalize(current.name)}.You can try again!`);

        if (failureCount.value >= 2) {
          pokemonStore.addEvent(`${capitalize(current.name)} ran away!`);
          pokemonStore.currentPokemon = null;
          failureCount.value = 0;
        }
      }
