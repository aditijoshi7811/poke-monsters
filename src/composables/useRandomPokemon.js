...
export function useRandomPokemon(pokemonStore) {
  ...
  const fetchRandomPokemon = async (generation) => {
    ...
    try {
      ...
      if (!genData) throw new Error(`Generation ${genId} data is not loaded; fetch it before requesting a random Pokémon.`);

      const speciesList = genData.pokemon_species || [];
      if (speciesList.length === 0) throw new Error(`No pokemon in generation ${genId}`);

      const randomSpecies = speciesList[Math.floor(Math.random() * speciesList.length)];

      // Use the store helper to fetch and set full Pokemon
      const pokemon = await pokemonStore.setCurrentPokemon(randomSpecies.name);
      return pokemon;
    } catch (err) {
